...
// Fetch posts for a course stream
router.get('/:courseId/posts', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.request()
      ...
      .query(`
        SELECT Id, CourseId, Author, Content, Timestamp
        FROM dbo.CoursePosts
...
// Post a new message to course stream
router.post('/:courseId/posts', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.request()
      ...
      .query(`
        INSERT INTO dbo.CoursePosts (CourseId, Author, Content)
        OUTPUT INSERTED.Id, INSERTED.CourseId, INSERTED.Author, INSERTED.Content, INSERTED.Timestamp
        VALUES (@courseId, @author, @content)
      `);
...
// Fetch a course by ID (with teachers)
router.get('/:courseId', async (req, res) => {
  ...
  try {
    ...
    res.json({ id: course_id, title, course_code, teachers });
  } catch (err) {
